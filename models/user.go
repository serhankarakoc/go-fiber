...
type User struct {
	...
	Password          string `gorm:"size:255;not null" json:"-"` // gizlenecek
	UserTypeID        uint   `gorm:"index" json:"user_type_id,omitempty"`
	ResetToken        string `gorm:"size:255;index" json:"-"`            // gizlenecek
	EmailVerified     bool   `gorm:"default:false;index" json:"email_verified"`
	VerificationToken string `gorm:"size:255;index" json:"-"`            // gizlenecek
